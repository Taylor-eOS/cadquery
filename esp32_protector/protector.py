...
def make_short_wall(tx):
    return (
        cq.Workplane("XY")
        .box(wall_thickness, case_y, top_height, centered=c)
...
def make_long_wall(ty):
    return (
        cq.Workplane("XY")
        .box(pcb_x, wall_thickness, top_height, centered=c)
...
def make_gap(ty):
    return (
        cq.Workplane("XY")
        .box(gap_width, wall_thickness * 3, top_height * 2, centered=c)
        .translate((0, ty, bottom_z))
    )

bottom_plate = (
    cq.Workplane("XY")
    .box(case_x, case_y, bottom_z, centered=c)
)

top_plate = (
    cq.Workplane("XY")
    .box(case_x, case_y, top_height, centered=c)
...
usb_wole = (
    ...
    .edges("|X")
    .fillet(usb_r)
    .translate((-side_wall_x, 0, usb_z + (usb_h / 2)))
)
